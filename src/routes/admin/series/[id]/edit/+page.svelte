...
<svelte:head><title>Edit {data.series.name} — Admin</title></svelte:head>
...
<div class="max-w-2xl space-y-6">
	<div class="flex items-center gap-3">
		...
		<div>
			<h1 class="text-2xl font-bold text-foreground">Edit Series</h1>
	...
	>
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div class="md:col-span-2">
				...
				<Input id="name" name="name" required value={s.name} placeholder="Series title" />
			</div>
			...
			<div>
				...
				<select id="status" name="status" class="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring">
					<option value="draft" selected={s.status !== 'published'}>Draft</option>
			...
			<div class="flex items-center gap-5 pt-5">
				<label class="flex items-center gap-2 cursor-pointer">
					<input type="checkbox" name="featured" class="rounded" checked={s.featured} />
					<span class="text-sm text-foreground">Featured</span>
				</label>
				<label class="flex items-center gap-2 cursor-pointer">
					<input type="checkbox" name="trending" class="rounded" checked={s.trending} />
				...
				<div class="flex flex-wrap gap-2">
					{#each data.genres as genre}
						<label class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-muted cursor-pointer hover:bg-muted/70 transition-colors">
							<input type="checkbox" name="genreIds" value={genre.id} checked={s.selectedGenreIds.includes(genre.id)} />
			...
			<div class="md:col-span-2">
				<FileUploader field="posterId" label="Poster Image" type="poster" existingId={s.posterId ?? ''} />
			</div>
		</div>
